...
int simPower                    = 12;          // SIM900 power pin is connected to this pin
int buttonState;                               // the current reading from the input pin
int lastButtonState             = HIGH;         // the previous reading from the input pin  - used for debouncing
long lastDebounceTime           = 0;           // the last time the output pin was toggled
long debounceDelay              = 50;          // the debounce time; increase if the output flickers

int numStartAddresses[] = { 
  10,21,32,43,54 };  //Starting address of stored numbers

void setup()
{
  pinMode(buttonPin, INPUT);
  pinMode(simPower, OUTPUT);
  pinMode(8,OUTPUT);    //led for testing and debugging
...
void checkPushButton()
{
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) {
    ...
    if (reading != buttonState) {
      ...
      if (buttonState == LOW && !interruptInProcess) {
        Serial.println("Push btn PRESSED");

        interruptInProcess = true;
        
        //makeCall();
        testCall();        //testCall is for testing ONLY, replace with makeCall in actual version
...
{
   //TODO: implement battery status reading
...
void handleConfigMsg(String configMsg)
{
  ...
  if ( storedNum.equals(sendersNum) || true)
  {
